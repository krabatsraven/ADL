...
## Structure
The architecture of the adl is kept track in the torch model `class AutoDeepLearner(nn.Module)`.
...
The base classifier `ADLClassifier(Classifier)` implemented in capymoa provides `train(x: instance) -> None`, `predict(x: instance) -> index of class` and `predict_proba(x: instance) -> class probabilities`
...
`extended_classifier(winning_layer_training, add_weight_correction_parameter_to_user_choices)` provides a (close to) canonical classifier.
...
### Classifier Features
A fabricator function for a classifier with the described functionalities is provided in `extended_classifier(*decorators: Callable[[type(ADLClassifier)], type(ADLClassifier)]) -> type(ADLClassifier)`.
...
#### Grace Period
`global_grace_period(duration: int = 1000)` and `grace_period_per_layer(duration: int = 1000)` both implement a grace period.
...
`extended_classifier(global_grace_period(256))` to add a global grace period of 256 instances.
...
`disabeling_deleted_layers` sets `requires_grad` of the corresponding hidden layer to False,  
and `delete_deleted_layers` omits the hidden layers activation function and deletes the hidden layer by merging the weightmatrix with the one of the layer behind it.  
The behaviour of both functionalities together is untested.

#### Adding a Voting Weight Learning Rate
Adds `layer_weight_learning_rate` as an additional hyperparameter to the kwargs of the adl classifier.
...
To use this simply add `add_weight_correction_parameter_to_user_choices()` to the args of the fabricator function.

#### Input Preprocessing
Adds input normalization as well as one-hot-encoding of the input instance before processing it.
To use this simply add `input_preprocessing` to the args of the fabricator function.
...
To use the vectorized version simply add `vectorized_for_loop` to the args of the fabricator function.
...
To employ winning layer training simply add `winning_layer_training` to the args of the fabricator function 
otherwise all active output layers and all hidden layers are trained using the optimizer.

### Evaluation
A fabricator function for a classifier with the described evaluation functionalities is provided in `extend_classifier_for_evaluation`.
The argument `with_emissions: bool = False` chooses whether the codecarbon framework is to be employed. ([Energy Recorder](#recorder-of-emitted-carbon-dioxide-and-used-energy))  
...
`record_network_graph` keeps track of the architecture adl possesses before it processes the instance.
...
Using Code Carbon `record_emissions` tracks the resources extended by the training in `_train`.
...
`run_bench` runs the tests needed for the thesis plots created by `ba_plots` in sequence, 
