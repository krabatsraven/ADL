...
if __name__ == "__main__":
    ...
    stream = ARFFStream(str(Path('data/electricity.arff').absolute().resolve()))
    ...
    while i < 10:
        ...
        print("-----")
        print("current: ", current_instance.x)
        print("transformed: ", out_tensor)
        print("amount of numerical features: ", len(numerical_indicies))
        print("amount of nominal features: ", len(nominal_indicies), " with ", sum([len(list(stream.schema.get_moa_header().attribute(i).getAttributeValues())) for i in nominal_indicies]), " classes")
